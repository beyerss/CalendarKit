...
class ViewController: UIViewController {
    ...
    @IBAction func showEmbeddedCalendar(sender: AnyObject) {
        if let calendar = embeddedCalendar {
            ...
            let config = CalendarConfiguration(displayStyle: .Custom, dateTextStyle: .TopLeft(verticalOffset: 6, horizontalOffset: 8), dateCircleSizeOffset: -7, monthHeaderFont: UIFont(name: "AmericanTypewriter-Bold", size: 30)!, dayHeaderFont: UIFont(name: "AmericanTypewriter", size: 13)!, dateLabelFont: UIFont(name: "AmericanTypewriter-Bold", size: 10)!, headerBackgroundColor: UIColor.purpleColor(), weekdayHeaderBackgroundColor: weekdayHeaderColor, dateHighlightColor: UIColor.purpleColor(), dateBackgroundColor: UIColor.whiteColor(), dateDisabledBackgroundColor: UIColor.darkGrayColor(), calendarBackgroundColor: UIColor.lightGrayColor(), dateTextEnabledColor: weekdayHeaderColor, dateTextDisabledColor: UIColor.whiteColor().colorWithAlphaComponent(0.3), dateTextSelectedColor: UIColor.whiteColor(), dateTextHighlightedColor: weekdayHeaderColor.colorWithAlphaComponent(0.6))
